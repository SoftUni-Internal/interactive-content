...
let VeterinaryClinic = result;
        ...
        let allProcedures = `All procedures by kind of pet "CAT"
Jim Jones
...
[test]
[input]
// Test 3 - If pet is registered and still waiting for their procedures you should throw an Error 
        let VeterinaryClinic = result;
        let clinic = new VeterinaryClinic('SoftCare', 10);
        clinic.newCustomer('Jim Jones', 'Tom', 'Cat', \['A154B', '2C32B', '12CDB'\]); 
...
[test]
[input]
// Test 4 - If pet is registered and still waiting for their procedures you should throw an Error", function () \{ 
        let VeterinaryClinic = result;
        let clinic = new VeterinaryClinic('SoftCare', 10);
        clinic.newCustomer('Jim Jones', 'Tom', 'Cat', \['A154B', '2C32B', '12CDB'\]); 
        expect(function() \{clinic.newCustomer('Jim Jones', 'Tom', 'Cat', \['A154B'\])\} ).to.throw(`This pet is already registered under Jim Jones name! Tom is on our lists, waiting for A154B, 2C32B, 12CDB.`);
