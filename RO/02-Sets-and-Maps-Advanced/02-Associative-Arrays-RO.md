...
În Java, matricele asociative se numesc Map. Map  **nu poate conține chei duplicate**, fiecare **cheie** poate asocia cel mult o valoare.

Dar **valorile pot fi duplicate**. Pentru o cheie, o singură valoare poate fi stocată în aceeași instanță Map.
...
HashMap este implementat ca **Hash Tablă** și nu există **nicio comandă pentru chei sau valori**.
...
**TreeMap** este **sortat** în funcție de **ordinea naturală a cheilor sale**.

**TreeMap** în Java **nu permite cheile nule**.

Este mai bine să utilizați un **TreeMap** când doriți **O Hartă își sortează perechile cheie-valoare după ordinea naturală a tastelor** (de exemplu, **ordine alfabetică** sau **ordine numerică**).TreeMap <Key, Value> 
...
**LinkedHashMap** mapează, de asemenea, o `cheie` și o `valoare`.

Moștenește clasa HashMap, dar **menține ordinea de inserare**.
...
Iterarea prin obiecte de tip **Map.Entry<K,V>**. **Nu se poate modifica** colecția (numai în citire).
...
- Iterarea prin elementele unei hărți utilizând metoda încorporată `entrySet()`
  - `entry.getKey()` - obține Cheile
  - `entry.getValue()` - obține Valorile
...
cars.entrySet()
    .stream()
    .sorted((a,b)->{
        // compareTo() - returns 0 if the elements are equal
        int sort = a.getValue().compareTo(b.getValue());

        // if the values are equal compare by the keys
        if (sort == 0){
            // sorting by the keys
            return a.getKey().compareTo(b.getKey());
        }
        // in case there are no equal values
        return sort;
    })
    .forEach(entry -> System.out.println(entry.getKey() + " - " + entry.getValue()));
