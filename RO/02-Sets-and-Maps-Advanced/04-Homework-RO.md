...
Printați rezultatul în **ordine alfabetică** (ordine lexicografică), utilizând sintaxa: `{simbol}: {număr} de ori`.
...
În cazul în care contactul nu e găsit, printați `Contact {nume} does not exist.`
...
Fiecare linie **impară** din consolă reprezintă o **resursă** (e.g.

Aur, Argint, Bronz și așa mai departe) și fiecare rând **par - cantitatea**.
...
Există un nou tip de amenințare în care utilizatorii inundă serverul cu mesaje care sunt greu de detectat, din moment ce își schimbă adresa IP tot timpul.

Doar un programator priceput poate găsi logările utilizatorului pe servere.

Datele de intrare se dau după formatul următor:

`IP=(adresaIP) mesaj=(unMesaj) utilizator=(numeUtilizator)`
...
Verificați exemplele de mai jos. Acestea vor clarifica și mai mult ceea ce vi se cere.
...
[task-description]
## Cerință
Atât de mulți oameni! Sunt greu de numărat.

Dar aceasta este sarcina voastră ca statisticieni.

Primiți date neprocesate pentru un anume oraș și trebuie să le agregați.

Pe fiecare linie, se dau date ce respectă următorul format: `oraș|țară|populație`. 
...
- Timpul destinat executării programului vostru: 0.1 secunde. Memorie alocată: 16 MB
...
- **< numeUtilizator >** constă doar în **caractere din alfabetul Latin**, de lungime [1 .... 20].
- **< durata >** e un număr întreg din intervalul [1 ... 1000].
- Limita de timp: 0.3 sec. Limita de memorie: 16 MB.
...
- **Shadowmourne** – necesită **250 cioburi**;
- **Valanyr** – necesită **250 fragmente**;
- **Dragonwrath** – necesită **250 fire**;
...
- Timp de execuție permis: 0.25s
- Memorie permisă: 16 MB
...
Valorile standard sunt următoarele: sănătate **250** , daune **45** și armură **10**. 
...
`-{Nume} -> daune: {daune}, sănătate: {sănătate}, armură: {armură}`
