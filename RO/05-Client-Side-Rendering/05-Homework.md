...
There is an **input field** with a **button**. 

In the input field you should enter **elements**, **separated** by а **comma** and а **whitespace** (", "). 

Create a simple **template** that creates a **list** of towns. 

The information about each **town** comes from the **input** field.

You **should** attach a click event to the **button** with id "**btnLoadTowns**" in your `attachEvents()` function and **render** the **towns** that come from the input field in the **HTML template** with "**towns-template**" id.
...
**Refactor** the **HTML** code and create a **template** to represent **each** cat **card block** on its own. 
...
The cats are **seeded** using the **function** from the js file called "**catSeeder.js**"
...
**This is how "Show status code"/"Hide status code" looks:**
...
Upon form submission, send a **POST** request to the same URL and if it is successful update the list of options adding the new item.

Each item has a **text** property entered by the user and "\_id", which is generated by the server. 
...
**This is how the rendered HTML should look:**
...
Create functionality that **searches** in a **table** by a provided input. 
...
When the "**Search**" **button** is **clicked**, go through all cells in the table and check if the given input is present. 
...
If we try to find all students who have email addresses in the "**john-dam**" domain the expected result is:
...
See the file **example.html** for a preview of how the rendered HTML may look like. 

You are free to add attributes that help you implement the required functionality.
...
Create functionality that creates a new book entry when the submit button is clicked. 

Before sending the request be sure the fields are not empty (validate the input). 

To **create** a book entry, you must send a "**POST**" request and the JSON body should be in the following format: 
...
## Update Book 
By clicking the edit button of a book entry, display the form with **id** "**edit-form**" and populate its fields with the information for the selected book:
