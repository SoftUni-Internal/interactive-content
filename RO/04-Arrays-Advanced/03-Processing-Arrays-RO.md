...
Începând cu ES5, tipul JavaScript **Array** furnizează metoda `map()` care ne permite **să transformăm elementele unei matrice** într-un mod mai curat.
...
În acest exemplu, avem o matrice **de șiruri** și, folosind `map()`, convertim fiecare element într-un **număr**. 
...
Metoda `filter()` creează o **nouă matrice** care conține toate elementele matricei care **trec un test dat** (furnizat ca funcție).
...
Scrieți o funcție care tipărește toate elementele de pe **poziții impare**, **dublate**, și **în ordine inversă**.

**Intrarea** este o matrice de numere.

**Ieșirea** este **tipărită pe consolă**, **pe o singură linie**, **separată printr-un spațiu.**
...
În funcție de **funcția de comparare** furnizată, ordinea de sortare poate **fi alfabetică sau numerică**, fie ascendentă (în sus) sau descendentă (în jos).

În mod implicit, metoda `sort()` sortează valorile ca **șiruri în ordine alfabetică și crescătoare.**

Dacă dorim să sortăm numere sau alte valori, trebuie să oferim o **funcție de comparare** corectă.

În acest exemplu, avem o matrice de nume.

Dacă apelăm metoda `sort()` direct, fără a transmite o **funcție de comparare**, metoda va funcționa corect - va sorta numele în ordine alfabetică crescătoare.
...
Fără **o funcție de comparare** transmisă, metoda `sort()` va trata matricea ca o matrice de **șiruri**, nu ca o matrice de **numere**.

Deci, le va sorta ca valori **de tip șir**. 
...
Rezultatul poate fi: **negativ, zero sau pozitiv.**

- Dacă rezultatul este negativ, `a` este sortat înainte de  `b`
...
Metoda `localeCompare()` compară două șiruri de caractere în localizarea curentă.

Este o **metodă de șir**, deci nu poate fi aplicată direct pe o matrice.

În acest exemplu, transmitem metoda `localeCompare()` **ca funcție de comparare**.
